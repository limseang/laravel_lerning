      ...
      <main class="flex-1">
       ...
       <div class="border-b border-gray-200 px-4 py-4 sm:flex sm:items-center sm:justify-between sm:px-6 lg:px-8">
          <div class="flex-1 min-w-0">
            <h1 class="text-lg font-medium leading-6 text-gray-900 sm:truncate">Functions</h1>
          </div>
          <div class="mt-4 flex sm:mt-0 sm:ml-4">
           
            <button  @click="add_funtion" type="button" class="order-0 inline-flex items-center px-4 py-2 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-purple-600 hover:bg-purple-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-purple-500 sm:order-1 sm:ml-3">Create Function</button>
          </div>
        </div>
          ...
          <div class="overflow-hidden shadow-sm ring-1 ring-black ring-opacity-5">
            <table class="min-w-full divide-y divide-gray-300">
              ...
              <tbody class="divide-y divide-gray-200 bg-white">
                <tr v-for="fnt in fnt" >
                 <td class="whitespace-nowrap px-9 py-4 text-sm text-gray-500">{{ fnt.id }}</td>
                  <td class="whitespace-nowrap py-4 pl-4 pr-3 text-sm font-medium text-gray-900 sm:pl-6 lg:pl-8">{{ fnt.name }}</td>
                 
                  <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{{ fnt.permission }}</td>
                  
                 
                     <td class="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6 lg:pr-8">
                    <a @click="editfnt(fnt.id)"  class="text-indigo-600 hover:text-indigo-900"
                   ...
                   <td class="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6 lg:pr-8">
                    <a href="" @click="deleteu(fnt.id)"  class="text-indigo-600 hover:text-indigo-900"
                      >Delete<span class="sr-only"><h1 v-if="deleteu">Vue is awesome!</h1>
</span></a
                    >
...
const add_funtion =()=>{
  router.push('/admin/add_function')

}

const deleteu =(id)=>{
  axios.post(`/delete/fnt/${id}/`,{
    params:{
      id:id,
  ...
  if(deleteu){
   alert('You Want to Delete Function?', 'Delete Functions', {
    confirmButtonText: 'yes', 
  cancelButtonText: 'no', type: 'warning',
      callback: action => {
    ...
    const getData = () => {
  axios.get('/back-end/fnt').then(res => {
